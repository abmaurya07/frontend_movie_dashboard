 ...
 */
const MovieStats: FC<MovieStatsProps> = ({ startYear, endYear }) => {
  ...
  return (
    <div className={`w-full max-w-screen-lg mx-auto ${UI_CONFIG.SPACING.SM} sm:${UI_CONFIG.SPACING.MD}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-3 sm:p-4 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center justify-between">
            <h3 className={`${UI_CONFIG.FONTS.SIZES.BASE} sm:${UI_CONFIG.FONTS.SIZES.LG} font-semibold text-gray-700`}>Average Rating</h3>
          ...
          <p className={`${UI_CONFIG.FONTS.SIZES.XS} sm:${UI_CONFIG.FONTS.SIZES.SM} text-gray-500 mt-1 sm:mt-2`}>average for {startYear}</p>
        </div>
      </div>
    </div>
  );
